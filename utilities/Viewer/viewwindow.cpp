...
void ViewWindow::OnMessage(dtCore::Base::MessageData* data)
{
   assert(data);
...
void ViewWindow::FileLoaded(bool loaded, const char* filename)
{
   if (!loaded)
   {
...
void ViewWindow::DisplayError(const std::string& msg)
{
   fl_alert("%s", msg.c_str());
}

const char* ViewWindow::GetPath(void)
{
   return mPath.c_str();
}
...
void ViewWindow::SetPath(const char* path)
{
...
void ViewWindow::Container(UserInterface* container)
{
   assert(container);
...
bool ViewWindow::LoadFile(const char* file)
{
...
bool ViewWindow::SaveFileAs(const char* filename)
{
...
void ViewWindow::SelectFile(unsigned int indx)
{
...
void ViewWindow::SetDisplay(unsigned int state, bool value /*= true*/)
{
   switch (state)
   {
...
void ViewWindow::SetMotion(unsigned int state)
{
   switch (state)
   {
      case ViewState::FLY:
...
void ViewWindow::SetJoystick(unsigned int state, bool value /*= true*/)
{
   switch (state)
   {
 ...
 */
void ViewWindow::CommandLine(int argc, char** argv)
{
   assert(mContainer);
...
void ViewWindow::KeyboardEventHandler(const KeyboardEvent& ev)
{
   assert(mContainer);
