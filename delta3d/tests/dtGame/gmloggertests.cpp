...
class TestControllerSignal: public sigslot::has_slots<>, public osg::Referenced
{
   public:
      ...
      void RegisterSignals(dtCore::RefPtr<dtGame::LogController> logController)
      { 
         ...
         //logController->_receivedStatus.connect_slot(this, &TestControllerSignal::OnReceivedStatus);
         //logController->_receivedRejection.connect_slot(this, &TestControllerSignal::OnReceivedRejection);
      } 
      void Reset() 
      {
...
{      
   public:         
      ...
      void Create(const std::string &logResourceName) { DoException(); }
      void Close() 
      { 
      ...
      void Open(const std::string &logResourceName) { DoException(); }
...
void GMLoggerTests::TestBinaryLogStreamTags()
{
   ...
   try
   {
      ...
      tag2 = tagList[0];
      CPPUNIT_ASSERT_MESSAGE("Tag unique ids should be equal",newTag == tag2);
...
void GMLoggerTests::TestPlaybackRecordCycle()
{
   ...
   try
   {
      ...
      mGameManager->AddComponent(*serverController);
      mGameManager->AddComponent(*logController);
