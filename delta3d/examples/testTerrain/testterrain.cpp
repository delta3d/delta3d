...
*/
class TestTerrainApp : public Application
{
   DECLARE_MANAGEMENT_LAYER( TestTerrainApp )
   ...
   */
   virtual void PostFrame(const double deltaFrameTime)
   {
      ...
      if ((mObjects.size() == 0) && (mSOARXTerrain->GetLCCStatus()))
      {
         ...
         //			dtUtil::PrintVisitor pv;
         //			mEnvironment->GetOSGNode()->accept(pv);

         //			dtUtil::SketchVisitor sv;
         //			mEnvironment->GetOSGNode()->accept(sv);

         //			dtUtil::ExtentsVisitor ev;
...
IMPLEMENT_MANAGEMENT_LAYER(TestTerrainApp)

int main( int argc, char **argv )
{
   SetDataFilePathList("..;" + GetDeltaDataPathList());

   TestTerrainApp* app = new TestTerrainApp( "config.xml" );
