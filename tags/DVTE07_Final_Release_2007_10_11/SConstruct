...
opts.AddOptions(
   EnumOption( 'mode', 'Build as either debug or release', 'release',
            ...
            map = {}, ignorecase = 1 ),
   BoolOption( 'pch', 'use a prefix/precompiled header', False ),
   BoolOption( 'buildPythonBindings', 'compile the python binding libraries', True ),
   BoolOption( 'buildExamples', 'compile the python binding libraries', True ),
   BoolOption( 'buildDemos', 'compile the python binding libraries', True ),
   BoolOption( 'fastBuild', 
     'Experimental option for full builds that combines the cpp files into one ' 
     'large one and builds them together.  This greatly speeds up building from scratch, '
     'but not necessarily incremental builds.  Don\'t use this with precompiled headers.', False ),
...
# additional libraries should be added here
...
#we still go into the examples dir because the example libraries are needed
#for the unit tests
SConscript( dirs=['examples'] )

if env.get("buildDemos"):
