...
namespace dtHLAGM
{
   ...
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_HLA_NAME_ELEMENT("hlaName");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_GAME_NAME_ELEMENT("gameName");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_HLA_DATATYPE_ELEMENT("hlaDataType");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_GAME_DATATYPE_ELEMENT("gameDataType");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_HLA_REQUIRED_ELEMENT("hlaRequired");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_GAME_REQUIRED_ELEMENT("gameRequired");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_DEFAULT_ELEMENT("default");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_ENUM_MAPPING_ELEMENT("enumerationMapping");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_ENUM_HLA_ID_ATTRIBUTE("id");
   const std::string HLAFOMConfigContentHandler::ONE_TO_ONE_ENUM_VALUE_ELEMENT("value");
      ...
      if (mInHLAConfig)
      {
         ...
         if (mInObjects)
         {
            if (mInObject)
            {
               ...
               {
                  mCurrentObjectToActor->GetOneToOneMappingVector().push_back(AttributeToProperty());
                  mCurrentAttrToProp = &mCurrentObjectToActor->GetOneToOneMappingVector().back();
               }
               else if (mCurrentAttrToProp != NULL)
               {
                  if (mParsingEnumMapping && sLocalName == ONE_TO_ONE_ENUM_VALUE_ELEMENT)
                  {
                     StartElementEnumMapping(sLocalName, attrs);                       
                  }
                  else if (sLocalName == ONE_TO_ONE_ENUM_MAPPING_ELEMENT)
                  {
                     if (mCurrentAttrToProp->GetGameType() == dtDAL::DataType::ENUMERATION)
                     {
                     ...
                     else
                     {
                        mLogger->LogMessage(dtUtil::Log::LOG_WARNING, __FUNCTION__, __LINE__,
                           ...
                           mCurrentAttrToProp->GetHLAName().c_str(), mCurrentAttrToProp->GetGameName().c_str());                        
                     }
                  }
               }
            }
            ...
            else if (sLocalName == OBJECT_ELEMENT)
            {
               ...
               if (itor != rMap.end())
               {
                  ...
                  if (superItor != mNamedObjectToActors.end())
                  {
                     ...
                     mCurrentObjectToActor->GetOneToOneMappingVector().insert(mCurrentObjectToActor->GetOneToOneMappingVector().end(),
                        super->GetOneToOneMappingVector().begin(), super->GetOneToOneMappingVector().end());
         ...
         else if (mInInteractions)
         {
            if (mInInteraction)
            {
               ...
               {
                  mCurrentInteractionToMessage->GetOneToOneMappingVector().push_back(ParameterToParameter());
                  mCurrentParamToParam = &mCurrentInteractionToMessage->GetOneToOneMappingVector().back();
               }
               else if (mCurrentParamToParam != NULL)
               {
                  ...
                  {
                     if (mCurrentParamToParam->GetGameType() == dtDAL::DataType::ENUMERATION)
                     {
                        mParsingEnumMapping = true;
                     }
                     else
                     {
                        mLogger->LogMessage(dtUtil::Log::LOG_WARNING, __FUNCTION__, __LINE__,
                           "Ignorning enumeration mapping for mapping \"%s\" to \"%s\" because the game datatype is not \"ENUMERATION\".",
                           mCurrentParamToParam->GetHLAName().c_str(), mCurrentParamToParam->GetGameName().c_str());                        
                     }
                  }
               }
            }
            else if (sLocalName == INTERACTION_ELEMENT)
            {
               ...
               if (itor != rMap.end())
               {
                  ...
                  if (superItor != mNamedInteractionToMessages.end())
                  {
                     ...
                     mCurrentInteractionToMessage->GetOneToOneMappingVector().insert(mCurrentInteractionToMessage->GetOneToOneMappingVector().end(),
                        super->GetOneToOneMappingVector().begin(), super->GetOneToOneMappingVector().end());
   ...
   void HLAFOMConfigContentHandler::StartElementEnumMapping(const std::string& elementName, const xercesc_dt::Attributes& attrs)
   {
      ...
      dtUtil::AttributeSearch::ResultMap rMap = as(attrs);
      //see if this object mapping extends another, i.e. inherits all attribute mappings.
      dtUtil::AttributeSearch::ResultMap::iterator itor = rMap.find(ONE_TO_ONE_ENUM_HLA_ID_ATTRIBUTE);
      if (itor != rMap.end())
      {
   ...
   void HLAFOMConfigContentHandler::ObjectToActorCharacters(const std::string& elementName, const std::string& characters)
   {
      ...
      {
         OneToOneCharacters(elementName, characters, *mCurrentAttrToProp);      
         return;
      }
   ...
   void HLAFOMConfigContentHandler::InteractionToMessageCharacters(const std::string& elementName, const std::string& characters)
   {
      ...
      {
         OneToOneCharacters(elementName, characters, *mCurrentParamToParam);      
         return;
      }
   ...
   void HLAFOMConfigContentHandler::OneToOneCharacters(const std::string& elementName, const std::string& characters, OneToOneMapping& mapping)
   {
      if (mParsingEnumMapping)
      {
         if (elementName == ONE_TO_ONE_ENUM_VALUE_ELEMENT)
         {
            mapping.AddEnumerationMapping(mCurrentEnumHLAID, characters);
         }            
      }      
      else if (elementName == ONE_TO_ONE_HLA_NAME_ELEMENT)
      {
         mapping.SetHLAName(characters);
      }
      else if (elementName == ONE_TO_ONE_HLA_DATATYPE_ELEMENT)
      {
         const AttributeType* dt = static_cast<const AttributeType*>(AttributeType::GetValueForName(characters)); 
         if (dt != NULL)
         {
      ...
      else if (elementName == ONE_TO_ONE_HLA_REQUIRED_ELEMENT)
      {
         mapping.SetRequiredForHLA(characters == "true" || characters == "1");
      }
      else if (elementName == ONE_TO_ONE_GAME_NAME_ELEMENT)
      {
         mapping.SetGameName(characters);
      }
      else if (elementName == ONE_TO_ONE_GAME_DATATYPE_ELEMENT)
      {
         const dtDAL::DataType* dt = static_cast<const dtDAL::DataType*>(dtDAL::DataType::GetValueForName(characters)); 
         if (dt != NULL)
         {
            mapping.SetGameType(*dt);
         }
         else
         {
            mLogger->LogMessage(dtUtil::Log::LOG_ERROR, __FUNCTION__, __LINE__,
      ...
      else if (elementName == ONE_TO_ONE_GAME_REQUIRED_ELEMENT)
      {
         mapping.SetRequiredForGame(characters == "true" || characters == "1");
      }
      else if (elementName == ONE_TO_ONE_DEFAULT_ELEMENT)
      {
         mapping.SetDefaultValue(characters);   
      }
