...
namespace dtAnim
{
   ...
   /////////////////////////////////////////////////////////////////////////////////////////////////
   bool SubmeshLODCullCallback::cull(osg::NodeVisitor* nv, osg::Drawable* drawable, osg::RenderInfo* renderInfo) const 
   { 
      osg::Node* parent = nv->getNodePath().back();
      if (parent != NULL)
      {
         float distance = nv->getDistanceToEyePoint(parent->getBound().center(), true);
         SubmeshDrawable* submeshDraw = static_cast<SubmeshDrawable*>(drawable);
     ...
     mWrapper(wrapper)
   {
      ...
      setCullCallback(new SubmeshLODCullCallback());
