...
void DialogTreeWidget::OnAddNewLine(QAction* action)
{
   ...
   if (lineType)
   {
      ...
      if (cur)
      {
         DialogLineItem* newLine = new DialogLineItem(lineType->GetName(), lineType, CreateIndex(), this);
...
void DialogTreeWidget::OnAddNewChoice()
{
   ...
   if (cur)
   {
      DialogChoiceItem* newChoice = new DialogChoiceItem(CreateIndex());
...
QWidget* DialogTreeDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
   ...
   if (item)
   {
      ...
      if (lineItem)
      {
         ...
         if (lineType)
         {
            return lineType->CreateInlineEditor(parent, mTree);
         }
      }
   }
...
////////////////////////////////////////////////////////////////////////////////
DialogLineItem::DialogLineItem(const QString& name, const DialogLineType* type, int index, DialogTreeWidget* tree)
   : mType(NULL)
   , mTypeName(name)
   , mTree(tree)
   , mIndex(index)
