...
namespace  dtCore
{
   ...
   void MapContentHandler::endDocument()
   {
      ...
      if (!mEnvActorId.ToString().empty())
      {
         try
         {
            dtCore::RefPtr<BaseActor> proxy = dynamic_cast<BaseActor*>(mMap->GetProxyById(mEnvActorId));
            if (!proxy.valid())
            {
      ...
      const xercesc::Attributes& attrs)
   {
      ...
      if (mInMap || mInPrefab)
      {
         ...
         else if (XMLString::compareString(localname, MapXMLConstants::HIERARCHY_ELEMENT_NODE) == 0)
         {
            ...
            dtCore::RefPtr<dtCore::BaseActor> nextNode = dynamic_cast<dtCore::BaseActor*>(mMap->GetProxyById(id));

            if (nextNode.valid())
            {
               mCurrentHierNode->AddChild(*nextNode);
               mCurrentHierNode = nextNode.get();
            }
         }
   ...
   void MapContentHandler::ActorCharacters(const XMLCh* const chars)
   {
      ...
      else if (topEl == MapXMLConstants::ACTOR_TYPE_ELEMENT)
      {
         ...
         BaseActor* parent = dynamic_cast<BaseActor*>(mBaseActorObject.get());
         ...
         if (mMissingActorTypes.find(actorTypeFullName) == mMissingActorTypes.end())
         {
            ...
            else
            {
               ...
               if (parent != NULL)
               {
                  BaseActor* actor = dynamic_cast<BaseActor*>(mBaseActorObject.get());
                  if (actor != NULL)
                  {
