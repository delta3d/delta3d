...
namespace dtCore
{
   typedef std::vector<dtCore::ActorComponent*> ActorComponentArray;
    ...
    */
   class DT_CORE_EXPORT ActorComponentContainer
   {
   public:
      ...
      {
         ActorComponentArray components = GetComponents(TComp::TYPE);
         if (!components.empty())
         {
   ...
   public:
       ...
       */
      virtual ActorComponentArray GetComponents(ActorComponent::ACType type) const = 0;

      /**
       * Fill the vector with all the actor components.
       */
      virtual void GetAllComponents(ActorComponentArray& toFill) = 0;
