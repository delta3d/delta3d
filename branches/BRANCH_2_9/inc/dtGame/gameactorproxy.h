...
namespace dtGame
{
   ...
   private:
      ...
      // Helper class to call OnEnteredWorld or OnRemovedFromWorld
      // for child GameActors or ActorComponents.
      struct ChildActorHandler
      {
         ChildActorHandler();

         // Method to call the appropriat OnEnteredWorld or OnRemovedFromWorld
         // method on the specified actor. The actor may be a GameActor or ActorComponent.
         bool HandleActor(BaseActor& actor);

         bool mCallEnteredWorld; // if FALSE, OnRemovedFromWorld will be called.
      };
