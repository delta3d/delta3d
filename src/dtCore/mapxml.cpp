...
namespace dtCore
{
   ...
   /////////////////////////////////////////////////////////////////////////////
   bool MapParser::ParsePrefab(const std::string& path, std::vector<dtCore::RefPtr<dtCore::BaseActorObject> >& actorList, dtCore::Map* map)
   {
   ...
   void MapWriter::Save(Map& map, std::ostream& stream)
   {
      ...
      try
      {
         ...
         {
            BaseActorObject& actor = *map.GetEnvironmentActor();
         ...
         for (; curIter != actorMap.end(); ++curIter)
         {
            ...
            if (actor->IsGhost() || (extendedActor != NULL && extendedActor->GetParent() != NULL))
            {
               continue;
            }
         ...
         dtCore::BaseActor* hier = map.GetDrawableActorHierarchy();
         if (hier != NULL)
         {
            BeginElement(MapXMLConstants::HIERARCHY_ELEMENT);
               for(int i = 0; i < hier->GetChildCount(); ++i)
               {
                  BaseActor* actor = hier->GetChild(i);
                  if (actor != NULL)
                  {
   ...
   /////////////////////////////////////////////////////////////////////////////
   void MapWriter::WriteHierarchyBranch(dtCore::BaseActor& actor)
   {
      ...
      ActorArray actors;
      actor.GetChildren(actors);

      dtCore::BaseActor* curActor = NULL;
      ActorArray::iterator curIter = actors.begin();
      ActorArray::iterator endIter = actors.end();
      ...
      BeginElement(MapXMLConstants::ACTOR_ELEMENT);
         ...
         if (extendedActor != NULL && extendedActor->GetChildCount() > 0)
         {
            BeginElement(MapXMLConstants::ACTOR_CHILDREN_ELEMENT);
               ...
               typedef dtCore::BaseActorArray ActorArray;
               ActorArray children;
               extendedActor->GetChildren(children);

               dtCore::BaseActorObject* childActor = NULL;
               ActorArray::iterator curChildIter = children.begin();
               ActorArray::iterator endChildIter = children.end();
               for (; curChildIter != endChildIter; ++curChildIter)
               {
                  childActor = curChildIter->get();

                  // RECURSE
                  results += WriteActor(*childActor, allowReadOnlyProps);
               }
